...
public  class DecodeAutoBlueFrontNew extends OpMode {
    ...
    final double LAUNCHER_TARGET_VELOCITY = 1610.0; // needs testing
    final double LAUNCHER_MIN_VELOCITY = 1510.0;

    double shotsToFire = 3;
    double TIME_BETWEEN_SHOTS = 2.0;    // reduced cycle time (tune)
    ...
    double robotRotationAngle = 75.0;
    ...
    public void loop() {
        ...
        switch (autonomousState) {
            ...
            case DRIVEALITTLE:
                if (drive(DRIVE_SPEED, -42.0, DistanceUnit.INCH, 0.5, 0)) {
            ...
            case ROTATING:
                ...
                break;
            case NEXTBALL:
                if (drive(0.8, 54.0, DistanceUnit.INCH, 0.5, 0) && driveModo == 0) {
                    driveModo = 1;
                ...
                if (drive(0.8, -54.0, DistanceUnit.INCH, 0.5, 0) && driveModo == 1) {
                    driveModo = 2;
                    limelightOn = true;
                }
                if (rotate(ROTATE_SPEED, 45.0, AngleUnit.DEGREES, 0.5) && driveModo == 2) {
                    driveModo = 3;
                ...
                if (driveModo == 4 && rotate(ROTATE_SPEED, odo.getHeading(AngleUnit.DEGREES), AngleUnit.DEGREES, 1.0)) {
                    frontLeft.setMode(DcMotorEx.RunMode.STOP_AND_RESET_ENCODER);
                    backLeft.setMode(DcMotorEx.RunMode.STOP_AND_RESET_ENCODER);
                    frontRight.setMode(DcMotorEx.RunMode.STOP_AND_RESET_ENCODER);
                    backRight.setMode(DcMotorEx.RunMode.STOP_AND_RESET_ENCODER);
                ...
                if (rotate(ROTATE_SPEED, -90, AngleUnit.DEGREES, 1.0) && driveModo == 6) {
                    driveModo = 7;
                    intakeMotor.setPower(1.0);
                    if (distanceInch < 500 || red < 25555 || green < 25555 || blue < 25555) {
                        leftFeeder.setPower(-1.0);
                        rightFeeder.setPower(1.0);
                    } else {
                        leftFeeder.setPower(0.0);
                ...
                if (drive(DRIVE_SPEED, 48.0, DistanceUnit.INCH, 0.5, 0) && driveModo == 7) {
                    driveModo = 8;
                    intakeMotor.setPower(0.0);
                    leftFeeder.setPower(0.0);
                    rightFeeder.setPower(0.0);
    ...
    boolean launch(boolean shotRequested) {
        ...
        switch (launchState) {
            ...
            case PREPARE:
                ...
                if (launcher.getVelocity() > (13.555 * distanceToGoalMm(alliance) / 25.4) || launcherSpinupTimer.seconds() > 5.0) {
            ...
            case PREPARE2:
                ...
                leftFeeder.setPower(-1.0);
                rightFeeder.setPower(1.0);
                if (boxServoTimer.seconds() > 5.0 || distanceInch > 500.0 || green < 25555 || blue < 25555 || red < 25555) {
                    boxServoTimer.reset();
                    shotTimer.reset();
    ...
    boolean rotate(double speed, double angle, AngleUnit angleUnit, double holdSeconds) {
        ...
        if (!limelightOn) {
            ...
            boolean done = (driveTimer.seconds() > 200.0);
